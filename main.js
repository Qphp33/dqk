...
async function getFofaTaskData(projectId, pageNum = 1, pageSize = 100, searchTerm = '', hostTypes = []) {
  try {
    ...
    if (searchTerm) {
      ...
      searchFields.forEach(() => params.push(fuzzy));
    }
...
async function getTaskData(projectId, pageNum = 1, pageSize = 100, searchTerm = '', targetFilter = '') {
  try {
    ...
    if (searchTerm) {
      conditions.push(`(host LIKE ? OR ip LIKE ? OR title LIKE ? OR fingerprint LIKE ? OR server LIKE ? OR domain LIKE ? OR region LIKE ? OR city LIKE ? OR port LIKE ? OR protocol LIKE ?)`);
      const fuzzy = `%${searchTerm}%`;
      params.push(fuzzy, fuzzy, fuzzy, fuzzy, fuzzy, fuzzy, fuzzy, fuzzy, fuzzy, fuzzy);
    }
