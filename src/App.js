...
const HomeDashboard = ({ refreshToken = 0 }) => {
  ...
  const maxRegionValue = summary.regionStats[0]?.value || 1;

  return (
    <div className="v17-home-dashboard">
      <div className="v17-home-hero">
        <div className="v17-home-hero-copy">
          <div className="v17-home-kicker">INTELLIGENCE OVERVIEW</div>
          <h1 className="v17-home-title">本地情报概览中心</h1>
          <p className="v17-home-subtitle">聚合任务、资产、FOFA 指纹与地区分布，让首页直接成为你的全局态势面板。</p>
        </div>
        <div className="v17-home-orbit">
          <div className="v17-home-orbit-ring"></div>
          <div className="v17-home-orbit-core">DATA</div>
        </div>
      </div>

      {loading ? (
        <div className="v17-home-loading">
          <RefreshCcw size={20} className="spinning" />
          <span>正在加载首页概览...</span>
        </div>
      ) : error ? (
        <div className="v17-home-error">{error}</div>
      ) : (
        <>
          <div className="v17-home-metrics">
            <div className="v17-metric-card">
              <div className="v17-metric-icon"><ListTodo size={18} /></div>
              <div className="v17-metric-label">任务数量</div>
              <div className="v17-metric-value">{formatCount(summary.taskCount)}</div>
            </div>
            <div className="v17-metric-card">
              <div className="v17-metric-icon"><Database size={18} /></div>
              <div className="v17-metric-label">任务数据数量</div>
              <div className="v17-metric-value">{formatCount(summary.taskDataTotal)}</div>
            </div>
            <div className="v17-metric-card">
              <div className="v17-metric-icon"><Bot size={18} /></div>
              <div className="v17-metric-label">指纹总量</div>
              <div className="v17-metric-value">{formatCount(summary.fingerprintRuleTotal)}</div>
            </div>
            <div className="v17-metric-card accent">
              <div className="v17-metric-icon"><Server size={18} /></div>
              <div className="v17-metric-label">IP 总量</div>
              <div className="v17-metric-value">{formatCount(summary.uniqueIpTotal)}</div>
            </div>
          </div>

          <div className="v17-home-grid">
            <section className="v17-panel v17-panel-wide">
              <div className="v17-panel-head">
                <div>
                  <h3>地区 IP 统计</h3>
                  <p>按本地任务详情中的地区数据聚合展示</p>
                </div>
                <div className="v17-panel-chip">
                  <MapPin size={14} />
                  <span>{formatCount(summary.uniqueIpTotal)} 个 IP</span>
                </div>
              </div>
              <div className="v17-region-bars">
                {summary.regionStats.length > 0 ? summary.regionStats.map((item, index) => (
                  <div className="v17-region-row" key={`${item.name}-${index}`}>
                    <div className="v17-region-meta">
                      <span className="v17-region-rank">{index + 1}</span>
                      <span className="v17-region-name">{item.name}</span>
                    </div>
                    <div className="v17-region-track">
                      <div className="v17-region-fill" style={{ width: `${Math.max(10, (item.value / maxRegionValue) * 100)}%` }}></div>
                    </div>
                    <div className="v17-region-value">{formatCount(item.value)}</div>
                  </div>
                )) : (
                  <div className="v17-empty">暂无地区数据</div>
                )}
              </div>
            </section>

            <section className="v17-panel">
              <div className="v17-panel-head">
                <div>
                  <h3>数据概览统计</h3>
                  <p>指纹规则总量与累计数据量</p>
              ...
              <div className="v17-stack-metrics">
                <div className="v17-stack-card">
                  <div className="v17-stack-card-icon"><Bot size={18} /></div>
                  <div className="v17-stack-card-info">
                    <span>指纹规则总量</span>
                    <strong>{formatCount(summary.fingerprintRuleTotal)}</strong>
                  </div>
                </div>
                <div className="v17-stack-card accent">
                  <div className="v17-stack-card-icon"><Activity size={18} /></div>
                  <div className="v17-stack-card-info">
                    <span>FOFA 数据总量</span>
                    <strong>{formatCount(summary.fofaFingerprintDataTotal)}</strong>
                  </div>
                </div>
              </div>
            </section>
