...
const ActionNoticeToast = ({ notice }) => {
  ...
  return (
    <div className={`v15-action-notice ${notice.variant || 'info'}`} role="status" aria-live="polite">
      <div className="v15-action-notice-icon">
        {notice.variant === 'success' ? <CheckCircle2 size={18} /> : notice.variant === 'error' ? <X size={18} /> : <Info size={18} />}
      </div>
...
const Tasks = ({ isOffline = false, canUseRemoteFeatures = false, refreshToken = 0 }) => {
  ...
  const [isTaskCloudDataSyncing, setIsTaskCloudDataSyncing] = useState(false);
  ...
  const openDetail = async (project) => {
    if (!project) return;
    if (project.step?.includes('运行') || project.step?.includes('running')) {
      pushActionNotice({
        variant: 'info',
        title: '暂不可查看详情',
        text: '任务运行中，需等待任务完成后才能进入详情页'
      });
      return;
    }
    const mainEl = document.querySelector('.content-area');
  ...
  const handleCloudSync = async () => {
    ...
    try {
      ...
      if (syncResult.success) {
        ...
        pushActionNotice({
          variant: 'success',
          title: '刷新完成',
          text: `已刷新 ${syncResult.count || 0} 个任务的最新状态`
        });
      } else {
        pushActionNotice({
          variant: 'error',
          title: '刷新失败',
          text: syncResult.error || '刷新任务状态失败'
        });
      }
    } finally {
    ...
    openModal({
      title: '刷新任务状态',
      message: '将从云端刷新任务列表和最新状态，不会覆盖本地任务详情数据，是否继续？',
      variant: 'info',
      okText: '立即刷新',
      cancelText: '取消',
      showCancel: true,
      onOk: async () => {
        ...
        try {
          ...
          if (syncResult.success) {
            ...
            openModal({
              title: '刷新完成',
              message: `已刷新 ${syncResult.count || 0} 个任务的最新状态`,
              variant: 'success',
              okText: '知道了',
              showCancel: false,
              onOk: () => { }
            ...
            openModal({
              title: '刷新任务状态失败',
              message: '从云端刷新任务状态失败：' + syncResult.error,
              variant: 'error',
              okText: '知道了',
              showCancel: false,
              onOk: () => { },
  ...
  const handleTaskCloudDataSync = async () => {
    if (!ensureRemoteAccess()) return;
    if (isTaskCloudDataSyncing) return;

    openModal({
      title: '同步云端数据',
      message: '将按当前任务 ID 从云端分页拉取详情数据，并覆盖写入本地数据库，是否继续？',
      variant: 'info',
      okText: '开始同步',
      cancelText: '取消',
      showCancel: true,
      onOk: async () => {
        setIsTaskCloudDataSyncing(true);
        const token = localStorage.getItem('token');
        try {
          const syncResult = await ipcRenderer.invoke('sync-task-cloud-data', { token });
          if (syncResult?.success) {
            ...
            setSyncingStatus({});
            if (detailProject?.id) {
              localStorage.removeItem(`synced_detail_${detailProject.id}`);
            }
            openModal({
              title: '同步完成',
              message: `已同步 ${syncResult.taskCount || 0} 个任务，共写入 ${syncResult.totalRows || 0} 条云端详情数据`,
              variant: 'success',
              okText: '知道了',
              showCancel: false,
              onOk: () => { }
            ...
            openModal({
              title: '同步云端数据失败',
              message: syncResult?.error || '同步云端数据失败，请稍后重试',
              variant: 'error',
              okText: '知道了',
              showCancel: false,
              onOk: () => { }
          ...
          setIsTaskCloudDataSyncing(false);
        }
      }
    });
  };
  ...
  const FofaAssetCard = ({ asset, onIconClick }) => {
    ...
    const fofaTagText = pickTagValue('fofaTag', 'fofa_tag');
    const subdomainTagText = pickTagValue('subdomainTag', 'subdomain_tag');
    const titleTagText = pickTagValue('titleTag', 'title_tag');
    const fingerprintTagText = pickTagValue('fingerprintTag', 'fingerprint_tag');
      ...
      <div className="v12-fofa-card">
        <div className="v12-card-main">
          <div className="v12-host-line">
            ...
            {fofaTagText && <span className="v12-data-tag">{fofaTagText}</span>}
          </div>
          ...
          <div className="v12-info-grid">
            ...
            {asset.domain && (
              <div className="v12-info-item">
                ...
                <span className="value mono v12-inline-value-with-tag">
                  <span>{asset.domain}</span>
                  {subdomainTagText && <span className="v12-data-tag">{subdomainTagText}</span>}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="v12-title-section">
            <div className="v12-title-row">
              {titleTagText && <span className="v12-data-tag">{titleTagText}</span>}
              <div className="v12-title-text" title={title}>{title}</div>
              <div className="v12-title-tags">
                {Object.entries(asset || {})
                  .filter(([key, value]) => key && /_tag$/i.test(key) && !['fofa_tag', 'subdomain_tag', 'title_tag', 'fingerprint_tag'].includes(String(key).toLowerCase()) && value !== null && value !== undefined && String(value).trim() !== '')
                  .map(([key, value], index) => {
                    const normalizedValue = String(value).trim();
                    const label = key.replace(/_tag$/i, '').replace(/[_-]+/g, ' ').trim();
                    return (
                      <span
                        key={`${key}-${index}`}
                        className="v12-data-tag"
                        title={label ? `${label}: ${normalizedValue}` : normalizedValue}
                      >
                        {normalizedValue}
                      </span>
                    );
        ...
        <div className="v12-card-side">
          <div className="v12-side-title-row">
            <div className="v12-side-title">指纹与组件</div>
            {fingerprintTagText && <span className="v12-data-tag">{fingerprintTagText}</span>}
          </div>
  ...
  const isDetailEntryDisabled = (project) => {
    const stepText = String(project?.step || '').toLowerCase();
    return stepText.includes('运行') || stepText.includes('running');
  };
  ...
  const getStepsStatus = (project) => {
    ...
    const stepText = String(project?.step || '').toLowerCase();
    const isDone = stepText.includes('\u5b8c\u6210') || stepText.includes('done') || stepText.includes('completed');
    const isRunning = stepText.includes('\u8fd0\u884c') || stepText.includes('running');
    const isError = stepText.includes('\u505c\u6b62') || stepText.includes('error') || stepText.includes('stopped');
    const currentStepIndex = steps.findIndex(step => stepText.includes(step.key));
    ...
    return (
      <div className="v18-related-panel">
        ...
        {filteredRows.length === 0 ? (
          <div className="v18-fp-empty-wrap">
            <DetailSectionEmpty title="\u6682\u65e0\u5339\u914d\u7684\u6307\u7eb9\u6570\u636e" sub="\u8bf7\u5c1d\u8bd5\u8c03\u6574\u6307\u7eb9\u540d\u79f0\u3001\u5206\u7c7b\u6216\u76ee\u6807\u7b5b\u9009\u6761\u4ef6\u3002" />
          </div>
        ) : (
          <div className="v18-related-table">
            <div className="v18-related-head" style={{ gridTemplateColumns: '1.8fr 1fr 1fr 1fr' }}>
              <div>{'\u6307\u7eb9\u540d\u79f0'}</div>
              <div>{'\u5206\u7c7b'}</div>
              <div>{'\u76ee\u6807\u6570'}</div>
              <div>{'\u8d44\u4ea7\u6570\u91cf'}</div>
            ...
            {filteredRows.map((row) => (
              <div
                key={row.name}
                className="v18-related-row"
                style={{ gridTemplateColumns: '1.8fr 1fr 1fr 1fr' }}
              ...
              >
                <div className="v18-related-domain-cell">
                  <span className="v18-related-domain clickable">{row.name}</span>
                </div>
                <div className="v18-related-source-cell">
                  <span className={`v18-fp-category-tag ${row.category.toLowerCase().replace('/', '-')}`}>{row.category}</span>
                </div>
                <div className="v18-related-source-cell">
                  <span className="v18-related-target-tag">
                    {Array.isArray(row.targets) ? `${row.targets.length} ${'\u4e2a\u76ee\u6807'}` : '-'}
                  </span>
                </div>
                <div className="v18-related-source-cell">
    ...
    <div className={`v6-tasks-container ${detailVisible ? 'detail-mode' : ''}`}>
      ...
      <div className="v6-toolbar">
        ...
        <div className="v6-actions">
          ...
          <button className={`v6-cloud-sync-btn ${isCloudSyncing ? 'syncing' : ''}`} onClick={handleCloudSync} title={isOffline ? '离线版本不可刷新任务状态' : '刷新任务状态'} disabled={isOffline}>
            <RefreshCw size={16} className={isCloudSyncing ? 'spinning' : ''} />
            <span>{isCloudSyncing ? '刷新中...' : '刷新任务状态'}</span>
          </button>
          <button className={`v6-cloud-sync-btn ${isTaskCloudDataSyncing ? 'syncing' : ''}`} onClick={handleTaskCloudDataSync} title={isOffline ? '离线版本不可同步云端数据' : '同步云端数据'} disabled={isOffline}>
            <CloudDownload size={16} className={isTaskCloudDataSyncing ? 'spinning' : ''} />
            <span>{isTaskCloudDataSyncing ? '同步中...' : '同步云端数据'}</span>
          </button>
          <div className="v6-help-tooltip-wrapper">
            <HelpCircle size={16} className="v6-help-icon" />
            <div className="v6-help-tooltip">刷新任务状态不会覆盖详情数据，同步云端数据会按任务 ID 覆盖本地详情表</div>
          </div>
          ...
          {viewMode === 'grid' ? (
            <div className="project-grid-wrapper">
              {filteredProjects.map((project) => (
                <div
                  key={project.id}
                  className={`v6-project-card ${openingDetailId === project.id ? 'is-opening' : ''} ${isDetailEntryDisabled(project) ? 'detail-disabled' : ''}`}
                  onClick={() => openDetail(project)}
                  title={isDetailEntryDisabled(project) ? '任务运行中，完成后才能查看详情' : '点击查看详情'}
                >
            ...
            <div className="v8-list-wrapper">
              ...
              <div className="v8-list-body">
                {filteredProjects.map((project) => (
                  <div
                    key={project.id}
                    className={`v8-list-row ${openingDetailId === project.id ? 'is-opening' : ''} ${isDetailEntryDisabled(project) ? 'detail-disabled' : ''}`}
                    onClick={() => openDetail(project)}
                    title={isDetailEntryDisabled(project) ? '任务运行中，完成后才能查看详情' : '点击查看详情'}
                  >
